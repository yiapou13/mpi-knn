...
/**** 1st communication and KNN calculation ****/
	...
	if (id == 0)
	{
		...
		MPI_Isend ( &(matrix[0][0]), m / procs * n, MPI_DOUBLE, dest, 0, MPI_COMM_WORLD, &sendrequest);	
		MPI_Irecv ( &(matrix_temp[0][0]), m / procs * (n + 2), MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &recvrequest );
	}
	else if (id == numprocs - 1)
	{
		...
		MPI_Isend ( &(matrix[0][0]), m / procs * n, MPI_DOUBLE, dest, 0, MPI_COMM_WORLD, &sendrequest);
		MPI_Irecv ( &(matrix_temp[0][0]), m / procs * n, MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &recvrequest );
	}
	else
	{
		int dest = id +1;
...
/**** N-th communication and KNN calculation ****/
	...
	for (int p = 0; p < procs - 1; p++)
	{
		...
		if (id == 0)
		{
			...
			MPI_Isend ( &(matrix_send[0][0]), m / procs * (n + 2), MPI_DOUBLE, dest, 0, MPI_COMM_WORLD, &sendrequest);
			MPI_Irecv ( &(matrix_temp[0][0]), m / procs * (n + 2), MPI_DOUBLE, source, MPI_ANY_TAG, MPI_COMM_WORLD, &recvrequest );
			printf("DONE\n");
		}
		else if (id == numprocs - 1)
...
/**** Matching labels ****/
	...
	if(id==0)printf("Time :%f", seq_time);
